...
export function buildLevel(levelPlan: string[]) {
  ...
  for (let y = 0; y < levelPlan.length; y++) {
    for (let x = 0; x < levelPlan[y].length; x++) {
      ...
      if (levelPlan[y][x] === 'S') {
        spikes.push(new PIXI.Point(x, y));
      }
    }
  }
  ...
  return { playerStart, AIStart, platforms, spikes, levelRect, flagPoint};
}
